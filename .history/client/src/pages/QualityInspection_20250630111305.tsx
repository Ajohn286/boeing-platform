...
const sampleImages: {
  ...
  {
    label: 'Aircraft Defect Analysis Video',
    src: '/aircraft defect analysis.mp4',
    type: 'video',
    defects: [
      // Simulated YOLO detections at certain seconds
...
export default function QualityInspection() {
  ...
  const [defects, setDefects] = useState<Defect[]>(sampleImages[0].defects);
  ...
  const handleSampleChange = (idx: number) => {
    ...
    setUploadedImg(null);
    setDefects(sampleImages[idx].defects.filter(d => !d.time));
    ...
    setTimeout(() => {
      if (selectedSample !== null) {
        ...
        if (sample.type === 'video') {
          // Only show defects for the current time
          setDefects(sample.defects.filter(d => Math.abs((d.time || 0) - currentVideoTime) < 1));
        } else {
          setDefects(sample.defects);
        }
  ...
  const handleVideoTimeUpdate = () => {
    if (videoRef.current) {
      ...
      // For simulation: update defects for current time
      if (selectedSample !== null && sampleImages[selectedSample].type === 'video') {
        setDefects(sampleImages[selectedSample].defects.filter(d => Math.abs((d.time || 0) - videoRef.current!.currentTime) < 1));
      }
    }
  };

  const displaySample = selectedSample !== null ? sampleImages[selectedSample] : null;
  const displayImg = uploadedImg || (displaySample && displaySample.type === 'image' ? displaySample.src : null);
  const displayVideo = displaySample && displaySample.type === 'video' ? displaySample.src : null;
  ...
  return (
    <div className="max-w-5xl mx-auto px-4 py-8">
      ...
      {/* Image/Video Upload & Sample Selector */}
      <div className="flex flex-col md:flex-row gap-4 mb-6 items-center">
        ...
        <button
          ...
          disabled={isLoading || (!displayImg && !displayVideo)}
        >
          {isLoading ? 'Running...' : 'Run Inspection'}
        </button>
      </div>

      {/* Inspection Media with Defect Overlay */}
            ...
            <table className="min-w-full text-xs border">
              <thead>
                <tr className="bg-gray-50">
                  ...
                  {displayVideo && <th className="px-3 py-2 text-left font-semibold">Timestamp (s)</th>}
                </tr>
              </thead>
              <tbody>
                {displayDefects.map((defect, idx) => (
                  <tr key={idx}>
                    ...
                    {displayVideo && <td className="px-3 py-2">{defect.time ?? Math.round(currentVideoTime)}</td>}
                  </tr>
                ))}
                {displayDefects.length === 0 && !isLoading && (
                  <tr><td colSpan={displayVideo ? 4 : 3} className="text-center text-gray-400 py-4">No defects detected yet.</td></tr>
                )}
              </tbody>
            </table>
          </div>
